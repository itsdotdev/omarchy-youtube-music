...
final class TrackCell: NSTableCellView {
    ...
    override func mouseEntered(with event: NSEvent) { mixButton?.alphaValue = 1 }
    override func mouseExited(with event: NSEvent) { mixButton?.alphaValue = 0 }
...
final class ProgressCell: NSSliderCell {
...
final class PlayerViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate, NSSearchFieldDelegate {
    ...
    override func loadView() {
        ...
        play.layer?.borderColor = NSColor.controlAccentColor.cgColor
    ...
    func render() {
        ...
        heading.stringValue = store.mixing ? "Building mix…" : "Up next"
